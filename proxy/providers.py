...
from common.smart_async_request import SmartAsyncRequest, SmartResponse
...
class Provider(ABC):
    ...
    @abstractmethod
    async def create_pages_urls(self) -> list[str]:
    ...
    @abstractmethod
    def parsing(self, resp: SmartResponse) -> list[str]:
    ...
    async def _run(self) -> None:
        pages_urls = await self.create_pages_urls()
        for url in pages_urls:
            resp = await self._request(url=url, timeout=5, retry=7)
            if not resp.error:
...
class FreeproxylistProxies(Provider):
    ...
    def parsing(self, resp: SmartResponse) -> list[str]:
        ...
        if resp.content:
            decode_content = resp.content.decode("utf-8")
            root = fromstring(decode_content)
...
class Api89ipcnProxies(Provider):
    ...
    def parsing(self, resp: SmartResponse) -> list[str]:
        ...
        if resp.content:
            decode_content = resp.content.decode("utf-8")
            hosts_ports = ip_address.findall(decode_content)
...
class XsdailiProxies(Provider):
    ...
    async def create_pages_urls(self) -> list[str]:
        ...
        for pagi_url in pagi_urls_list:
            resp = await self._request(url=pagi_url, timeout=5)
            if resp.content:
                decode_content = resp.content.decode("utf-8")
                root = fromstring(decode_content)
...
class GithubAccountProxies(Provider):
    ...
    def parsing(self, resp: SmartResponse) -> list[str]:
        ...
        if resp.content:
            decode_content = resp.content.decode("utf-8")
            list_of_tuple_host_port = re.findall(IP_PORT_PATTERN, decode_content)
